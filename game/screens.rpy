...
screen role_select_screen():
    modal True # 禁止点击背景
    
    # --- 核心修复：定时器 ---
    # 每 0.1 秒调用一次检查函数
    # 如果函数返回了非空值（即有结果了），则执行 Return() 退出屏幕
    timer 0.1 repeat True action Function(role_selection_periodic_check) 
    
    # 另一种情况：检查函数的返回值如果赋值给了 result，可以通过 if 判断来 Return
    # 但 Ren'Py 的 timer 比较特殊，我们结合 Variable 监听
    
    # 更直接的方法：如果 my_role 已经不是 0 了，直接退出
    ...
    vbox:
        ...
        text "先点先得，手慢无！" xalign 0.5 size 40 color "#fff"
        ...
        textbutton "我想当魔王":
            ...
            action [
                Function(player_select_role, ROLE_KING),
                Return() 
            ]
            sensitive (my_role == ROLE_NONE)
        ...
        textbutton "我想当勇者":
            ...
            hover_background "#555"
            action [
                Function(player_select_role, ROLE_HERO),
                Return()
            ]
            sensitive (my_role == ROLE_NONE)

        if my_role == ROLE_KING:
            text "正在进入：魔王线..." color "#f00" xalign 0.5
        elif my_role == ROLE_HERO:
            text "正在进入：勇者线..." color "#00f" xalign 0.5
        else:
            text "等待选择..." color "#aaa" xalign 0.5
...
screen navigation():
    ...
    vbox:
        ...
        # 修改这里：
        # 1. xpos: 决定按钮组的中心轴在哪里。
        #    试着填入 240 (如果是1080p屏幕) 或 160 (如果是720p屏幕)
        #    如果觉得偏左，就加大这个数；觉得偏右，就减小这个数。
        xpos 240  

        # 2. xanchor: 必须保持 0.5，这代表“以中心为基准”
        xanchor 0.5 
        # -------------------------------------------------
...
style navigation_button:
    size_group "navigation" # 让所有按钮点击区域大小一致
...
style navigation_button_text:
    ...
    outlines [(3, "#1b0d24", 0, 0)] 

    hover_outlines [(3, "#180a21", 0, 0), (6, "#391241ff", 0, 0)]
...
transform notify_appear:
    # 从屏幕顶端滑入
    ypos -0.1  # 使用比例值，使其在不同分辨率下表现更一致
    # 动画到屏幕高度 1/3 的位置
    linear 0.3 ypos 0.18
    pause 2.5
    # 平滑淡出
...
screen notify(message):
    ...
    # 将 frame 应用 (at) 我们定义好的 transform 动画
...
screen waiting_screen(message="等待中..."):

    # zorder 确保这个界面显示在最上层
    zorder 200

    # 一个半透明的黑色背景，让玩家知道游戏处于“暂停”状态
    add "gui/overlay/main_menu.png" alpha 0.5
    ...
    frame:
        ...
        # 文本框样式
        background Frame("gui/frame.png", 10, 10) # 你可以换成你自己的UI框体图片
        padding (30, 10)
